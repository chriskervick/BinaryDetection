...
def myloglike(cube):
    c,g1,g2,sb = cube[0],cube[1],cube[2],cube[3]
    #c,g1,g2,sb,L = cube[0],cube[1],cube[2],cube[3],cube[4]
    ans = np.sum(np.log(PlummerSep(s,a,Np) + UniCircle(s,rmax,Nu)+2*CrossTerm(s,a,rmax,Np,Nu) + 0.5*BPL(s,g1,g2,sb,L)*c))
    ...
    ans=ans-numsamples*np.log(0.5*c*intbpl(smin,smax,g1,g2,sb,L) + sepsinteg)
    return(ans)

# number of dimensions our problem has
#parameters = ["c","g1","g2","sb",'L']
parameters = ["c","g1","g2","sb"]
...
mean_val =np.mean(resforme[1,:])
mean_std =np.std(resforme[1,:])
sigma_val =np.mean(resforme[2,:])
sigma_std =np.std(resforme[2,:])
hc_val =np.mean(resforme[3,:])
hc_std =np.std(resforme[3,:])
#es_val =np.mean(resforme[4,:])
#es_std =np.std(resforme[4,:])



resarray = np.array([[c_val,c_std],[mean_val,mean_std],[sigma_val,sigma_std],[hc_val,hc_std]])#,[es_val,es_std]])
